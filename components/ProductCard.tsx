...
export function ProductCard({
  ...
  // Deterministic seed for reviews
  ...
  // Fixed "random" reviews between 50 and 100 based on product ID
  ...
  const content = (
    <>
      ...
      >
        ...
        <div
          className={`absolute bottom-0 inset-x-0 bg-white/95 backdrop-blur-sm p-3 translate-y-full transition-transform duration-500 flex flex-wrap gap-1 justify-center ${isHovered ? "translate-y-0" : ""}`}
        >
          <span className="text-[8px] font-black uppercase tracking-widest text-black/40 w-full text-center mb-1">
          ...
          product.collection?.toLowerCase().includes("fragrance")
            ...
            .map((size) => (
              <span
                key={size}
                className="text-[9px] font-bold border border-black/10 px-1.5 py-0.5 rounded text-black/70"
              >
                {size}
              </span>
            ))}
      ...
      <div className="flex flex-col p-2 pb-4 flex-grow md:p-5 border-black bg-[#f1f1f1] ">
        ...
        <div className="mt-auto space-y-1">
          <div className=" flex justify-between gap-3">
            ...
            {discount > 0 && (
              <div className="  text-black/40 text-[9px] font-black px-2 py-1 uppercase tracking-luxury ">
                -{discount}% OFF
              </div>
            )}
          </div>
          ...
          <div className="flex items-baseline gap-1.5">
            {hasPriceVariation && (
              <span className="text-[9px] text-accent font-black uppercase tracking-widest italic">
                From
              </span>
            )}
  ...
  const baseClassName =
    "group flex flex-col bg-white overflow-hidden transition-all duration-700 hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)] no-underline cursor-pointer border border-black/5";
